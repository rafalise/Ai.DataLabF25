...
def robust_range(s: pd.Series):
    ...
    if s.notna().sum()==0: return (0.0, 1.0)
...
if DEM_FILE.exists():
    ...
    for c in ["tot_pop","white_pct","black_pct","asian_pct","aian_pct","nhpi_pct","two_plus_pct","hispanic_pct"]:
        if c in demo.columns: demo[c] = pd.to_numeric(demo[c], errors="coerce")
    tbl = tbl.merge(
...
def make_map(df: pd.DataFrame, geojson, metric: str, outlines: bool) -> go.Figure:
    ...
    fig = px.choropleth(
        plot, geojson=geojson, locations="county_fips", featureidkey="id",
        color=metric, color_continuous_scale=palette,
        hover_name="county_name", hover_data={}
    )
    fig.update_traces(
        hovertemplate=plot["hover_text"],
        marker_line_width=(0.6 if outlines else 0), marker_line_color="#777"
    )
...
def update_details(fips):
    ...
    d_dens  = f"{row['np_density_10k']:.2f}" if pd.notna(row['np_density_10k']) else "N/A"
